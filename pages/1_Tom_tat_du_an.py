...
# Inject CSS để áp dụng định dạng kiểu chữ Roboto đồng bộ toàn trang
inject_custom_css()

# Banner tiêu đề chính của dự án CDRI Fellowship 2025–2026
st.markdown("""
    <style>
        .hero-banner {
        ...
        /* Cấu hình hiển thị tối ưu tự động cho giao diện Tối (Dark Mode) của Streamlit */
...
# Hệ thống các cột nút nhấn truy cập nhanh cổng thông tin liên kết bên ngoài
...
# Tạo cấu trúc các Tab nội dung trang chủ dự án
...
with tab_over:
    ...
    st.markdown('<div class="academic-paragraph">Dự án nghiên cứu <i>"Bản đồ rủi ro ngập lụt theo ngành nhằm tăng cường ứng phó và thích ứng với biến đổi khí hậu tại Thành phố Huế" </i> ứng dụng công nghệ viễn thám, hệ thống GIS và khảo sát thực địa để lượng hóa năng lực chống chịu của 32 cơ sở giáo dục và y tế tại các phường hành chính khu vực trung tâm thành phố Huế. Nghiên cứu tích hợp các nguồn dữ liệu này để xây dựng Bản đồ phơi nhiễm ngập lụt kết hợp Chỉ số tổn thương lũ (Flood Vulnerability Index – FVI) cấp cơ sở.</div>', unsafe_allow_html=True)
    
    st.markdown("""
        <div class="highlight-box">
            <b>Không phải mọi cơ sở nằm trong cùng một vùng ngập đều có mức độ tổn thương giống nhau.</b> <br><br>
    ...
    # Khu vực chỉ số KPI Card hiển thị tham số tổng hợp
    ...
    col_idx1.metric("Cơ sở được đánh giá", "32", "22 Trường học | 10 Cơ sở y tế")
...
with tab_contrib:
    ...
    st.markdown("""
        <ol>
            ...
            <li class="academic-paragraph"><b>Đánh giá chi tiết ở cấp từng cơ sở:</b> Tính toán điểm tổn thương cho từng trường học và cơ sở y tế một cách độc lập.</li>
            <li class="academic-paragraph"><b>Tích hợp đa nguồn dữ liệu tiên tiến:</b> Kết hợp dữ liệu vệ tinh radar Sentinel-1, GIS địa hình cao độ số và điều tra bảng hỏi thực địa 32 cơ sở hạ tầng thiết yếu.</li>
        </ol>
    """, unsafe_allow_html=True)

# --- TAB DỰ ÁN NGHIÊN CỨU ---
with tab_overview_proj:
    ...
    with col_proj1:
        st.markdown("""
            ...
            |---|---|
            | **Khu vực nghiên cứu** | Các phường khu vực trung tâm thành phố Huế |
            | **Đối tượng** | 22 trường học và 10 cơ sở y tế |
            | **Quy mô khảo sát** | 32 cơ sở hạ tầng thiết yếu |
    ...
    with col_proj2:
        st.markdown("""
            <div class="highlight-box" style="margin-top:0px;">
                ...
                Ảnh viễn thám Sentinel-1 ➔ Hệ thống thông tin địa lý (GIS) ➔ Khảo sát thực địa trực tiếp ➔ Chuẩn hóa dữ liệu & Phân tích toán học ➔ Chỉ số tổn thương lũ (FVI).
            </div>
        """, unsafe_allow_html=True)

# --- TAB KẾT QUẢ VÀ GIÁ TRỊ ỨNG DỤNG ---
with tab_application:
    st.markdown('<div class="sub-section-title">Kết quả nổi bật đạt được</div>', unsafe_allow_html=True)
    st.markdown('<div class="academic-paragraph">Nghiên cứu đã xây dựng thành công bản đồ phơi nhiễm ngập lụt độ phân giải 10 m và Chỉ số tổn thương lũ cho 32 trường học và cơ sở y tế tại thành phố Huế. Mô hình phát hiện vùng ngập đạt độ chính xác 95–97%, tạo nền tảng cho việc đánh giá rủi ro ở cấp từng cơ sở. Kết quả phân tích phân định rõ mức độ rủi ro không gian, đồng thời làm rõ các yếu tố chính chi phối kỹ thuật ảnh hưởng đến mức độ tổn thương của từng cơ sở hạ tầng.</div>', unsafe_allow_html=True)
    ...
    st.markdown("""
        <ul>
            ...
            <li class="academic-paragraph"><b>Đối với ngành giáo dục và y tế:</b> Cung cấp dữ liệu định lượng cụ thể để từng đơn vị rà soát các hạn chế về cơ sở vật chất, trang bị (như thiếu máy bơm, thiếu máy nổ dự phòng, thiếu nhân lực phản ứng khẩn cấp) để lên phương án thích ứng mùa mưa lũ.</li>
